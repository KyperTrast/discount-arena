...
// Kyper - Discount Arena - ChaseNext modified to skip deadflag
void ArenaChaseNext(edict_t* ent)
{
	ptrdiff_t i, j;
	edict_t* e;
	...
	i = j = ent->client->chase_target - g_edicts;
	...
	do
	{
		...
		if (!e->inuse)
			continue;
		if (!e->client->resp.spectator && !e->deadflag)
			break;
		if (i == j)
			return;
	} while (e != ent->client->chase_target);
...
// GetChaseTarget modified to look for someone that is alive and not ourself
void GetArenaChaseTarget(edict_t* ent)
{
	uint32_t i;
	edict_t* other;
	...
	{
		other = g_edicts + i;
		if (other->inuse && !other->client->resp.spectator && !other->deadflag && other != ent)
		{
			ent->client->chase_target = other;
			ent->client->update_chase = true;

			ent->client->ps.pmove.pm_flags = PMF_NONE;

...
void CTFSpawn()
{
	...
	// Kyper - Discount Arena
	// Initialize other vars - is there a better place to do this?
	if (g_arena_enabled)
	{
		// override some cvars that may have been set by default and added to player configs as part of base game
		// kind of messy...
		if (matchstarttime->integer == 20)
		{
			gi.cvar_set("matchstarttime", "9");
			matchstarttime = gi.cvar("matchstarttime", "9", CVAR_NOFLAGS);
		}
...
void CTFTeam_f(edict_t *ent)
{
	...
	if ((ctfgame.match > MATCH_SETUP && !g_arena_enabled) || (g_arena_enabled && CTFArenaInRound()))  // Kyper - Discount Arena - loosen team switching to just during round
	{
...
{
	// Kyper - Discount Arena - display Discount Rocket Arena
	if (g_arena_enabled)
		Q_strlcpy(p->text, "Discount Rocket Arena", sizeof(p->text));
	// Kyper
	else if (ctf->integer)
...
{
	const char *generic_help[] = { "\nDiscount Rocket Arena overview:\n",
								  "==================================\n",
								  "Work with your team to frag the other team.\n",
								  "You start with all base game weapons except BFG.\n",
								  "Respawn at round end, when one (or both) teams are fragged.\n",
								  "\n",
								  "Cvars:\n",
								  "================\n",
...
void CTFUpdateJoinMenu(edict_t *ent)
{
	...
	{
		if (ctfgame.match >= MATCH_PREGAME && !g_arena_enabled)        // Kyper - Discount Arena
		{
...
// Kyper - Discount Arena
bool CTFArenaInPreround()
{
	if (ctfgame.match == ARENA_PREROUND)
		return true;
	return false;
}
...
{
	int t = (level.time - (ctfgame.matchtime - gtime_t::from_min(matchtime->value))).seconds<int>();
...
bool CTFArenaCheckRules()
{
	...
	if (ctfgame.match == ARENA_PREROUND || ctfgame.match == ARENA_ROUND || ctfgame.match == ARENA_SETUP)
	{
		...
		int team1 = 0, team2 = 0;
		int team1_alive = 0, team2_alive = 0;
		int team1_health = 0, team2_health = 0;
		...
		{ // time ended on something
			...
			case ARENA_ROUND:
				...
				// and if completely the same, no points for anyone
				for (i = 1; i <= game.maxclients; i++)
				{
					ent = g_edicts + i;
					if (!ent->inuse)
						continue;
					if (ent->client->resp.ctf_team == CTF_TEAM1 && !ent->deadflag)
					{
						team1_alive++;
						team1_health += ent->health;
					}
					else if (ent->client->resp.ctf_team == CTF_TEAM2 && !ent->deadflag)
					{
						team2_alive++;
						team2_health += ent->health;
					}
				}
		...
		case ARENA_ROUND:
			G_FmtTo(text, "MATCH TIME REMAINING: {:02}", t % 60);
			gi.configstring(CONFIG_CTF_MATCH, text);
			...
			if (ctfgame.countdown)
			{
				int endtime = (ctfgame.matchtime - level.time).seconds<int>();
	...
	return false;  // Kyper - Discount Arena - no idea how I want to handle ending map, just let timelimit handle it...
}

...
void ClientObituary(edict_t *self, edict_t *inflictor, edict_t *attacker, mod_t mod)
{
	...
	if (base)
	{
		...
		if (deathmatch->integer && !mod.no_point_loss)
		{
			...
			if (teamplay->integer && !g_arena_enabled)  // Kyper - Discount arena - don't reduce score on world death
		...
		if (deathmatch->integer)
		{
			if (mod.friendly_fire)
			{
				if (!mod.no_point_loss)
				{
					...
					if (teamplay->integer && !g_arena_enabled)  // Kyper - Discount Arena - don't modify team score from death
						G_AdjustTeamScore(attacker->client->resp.ctf_team, -1);
				}
			}
			else
			{
				// Kyper - Discount Arena - no points in intermission
				if (!g_arena_enabled || (g_arena_enabled && CTFArenaInRound()))
					attacker->client->resp.score++;
				// Kyper

				if (teamplay->integer && !g_arena_enabled)     // Kyper - Discount Arena - don't modify team score from kill
					G_AdjustTeamScore(attacker->client->resp.ctf_team, 1);
			}
		}
...
void TossClientWeapon(edict_t *self)
{
	...
	// Kyper - Discount Arena - no dropping!
...
void InitClientPersistant(edict_t *ent, gclient_t *client)
{
	...
	client->pers.health = g_arena_start_health->integer;
	client->pers.max_health = g_arena_start_health->integer;
	// Kyper
		...
		(!G_TeamplayEnabled() && !client->resp.spectator))
	{
		...
		if (!taken_loadout)
		{
			...
			// Kyper - Discount Arena
			else if (g_arena_enabled->integer)
			{
				...
				client->pers.inventory[IT_ARMOR_BODY] = g_arena_start_armor->integer;
			}
			// Kyper
...
void respawn(edict_t *self)
{
	if (deathmatch->integer || coop->integer)
	{
		...
		// Kyper - Discount Arena - prevent respawning if in middle of a round
		if (CTFArenaInRound())
		{
			GetArenaChaseTarget(self);
			return;
		}
		// Kyper
...
// Kyper - Discount Arena
// Respawn players on the start of a new round
void arena_respawn(edict_t* ent)
{
	// leave a dead body in case we didn't click anything before next round	
...
void PutClientInServer(edict_t *ent)
{
	...
	// Kyper - Discount Arena - prevent respawn during active round
	if (CTFArenaInRound())
	{
		// start as 'observer'
		ent->movetype = MOVETYPE_NOCLIP;
...
bool ClientConnect(edict_t *ent, char *userinfo, const char *social_id, bool isBot)
{
	...
	if ( isBot ) {
		...
		// Kyper - Discount Arena - prefix bots with [BOT]
...
void ClientThink(edict_t *ent, usercmd_t *ucmd)
{
	...
	// Kyper - Discount Arena - prevent firing in arena pre-round state
	...
	{
		if (client->resp.spectator || (CTFArenaInRound() && ent->deadflag && ent->client->chase_target))  // Kyper - Discount Arena - chase target if dead
		{
			...
			if (client->chase_target)
			{
				...
				GetArenaChaseTarget(ent);   // Kyper - Discount Arena - only look for targets that are alive and not us
		}
	...
	// Kyper - Discount Arena - if died in an arena, allow cycling of chase targets, but only if we are currently chasing (this can bypass CopyToBodyQue)
	if (client->resp.spectator || (CTFArenaInRound() && ent->deadflag && client->chase_target))
	// Kyper
	{
		if (!HandleMenuMovement(ent, ucmd))
		{
			if (ucmd->buttons & BUTTON_JUMP)
			{
				if (!(client->ps.pmove.pm_flags & PMF_JUMP_HELD))
				{
					...
					if (client->chase_target)
						ArenaChaseNext(ent);         // Kyper - Discount Arena - only cycle to targets that are alive
					else
						GetArenaChaseTarget(ent);    // Kyper - Discount Arena - only look for targets that are alive and not us
				}
			}
